...
The EDGE gate-decision log is a second internal, non-public pilot store, written to `data/runtime/edge/gate_decisions.jsonl` and ignored by Git just as the SQLite ledger is. It records the analyst's own pursue/pass/watch call made *after* a packet — the decision, the decision-time confidence, and the factors behind it — never a person, never a packet score. A decider may be named only by an opt-in controlled team alias, only by that decider for their own call; the log is aggregate-only and is never rendered as a per-person or per-decider comparison. Like the ledger, it is not suitable for CUI/FCI or multi-user production, and any real (non-pilot) use requires the same approved identity, authorization, retention, and audit controls. See `docs/edge/GATE_DECISION_LOG.md` and ADR-027.
